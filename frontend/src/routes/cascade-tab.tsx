...
import { useCascadeUrl } from "#/hooks/query/use-cascade-url";
import { VSCODE_IN_NEW_TAB } from "#/utils/feature-flags";

function CascadeTab() {
  const { t } = useTranslation();
  const { data, isLoading, error } = useCascadeUrl();
    ...
    return (
      <div className="w-full h-full flex items-center text-center justify-center text-2xl text-tertiary-light">
        {iframeError ||
          data?.error ||
          String(error) ||
          t(I18nKey.CASCADE$URL_NOT_AVAILABLE)}
      </div>
    );
  }
...
// Export the Cascade directly since we're using the provider at a higher level
